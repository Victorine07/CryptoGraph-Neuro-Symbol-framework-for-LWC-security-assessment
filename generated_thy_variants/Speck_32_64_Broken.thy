...
definition speck_alpha :: nat where "speck_alpha = 7"


definition speck_beta :: nat where "speck_beta = 2"


definition speck_encrypt_round :: "16 word ⇒ 16 word × 16 word ⇒ 16 word × 16 word" where
...
definition speck_decrypt_round_inverse :: "16 word ⇒ 16 word × 16 word ⇒ 16 word × 16 word" where
  "speck_decrypt_round_inverse k xy_new = (
    let (x, y) = xy_new;
        xor_xy = xor x y;
        new_y = word_rotr 2 xor_xy;
        xor_xk = xor x k;
        msub = xor_xk - new_y;
        new_x = word_rotl 7 msub
    in (new_x, new_y))"


function speck_gen_key_schedule_rec :: "16 word list ⇒ 16 word list ⇒ nat ⇒ 16 word list" where
...
definition speck_generate_key_schedule :: "16 word list ⇒ 16 word list" where
...
fun speck_encrypt_iterate :: "16 word × 16 word ⇒ 16 word list ⇒ 16 word × 16 word" where
  "speck_encrypt_iterate state [] = state"
| "speck_encrypt_iterate state (k#ks) = speck_encrypt_iterate (speck_encrypt_round k state) ks"


fun speck_decrypt_iterate :: "16 word × 16 word ⇒ 16 word list ⇒ 16 word × 16 word" where
  "speck_decrypt_iterate state ks = foldl (λst_new k. speck_decrypt_round_inverse k st_new) state (rev ks)"


definition speck_encrypt_block :: "16 word × 16 word ⇒ 16 word list ⇒ 16 word × 16 word" where
  "speck_encrypt_block state keys = speck_encrypt_iterate state keys"


definition speck_decrypt_block :: "16 word × 16 word ⇒ 16 word list ⇒ 16 word × 16 word" where
  "speck_decrypt_block state keys = speck_decrypt_iterate state keys"


definition speck_encrypt :: "32 word ⇒ 16 word list ⇒ 32 word" where
...
definition speck_decrypt :: "32 word ⇒ 16 word list ⇒ 32 word" where
