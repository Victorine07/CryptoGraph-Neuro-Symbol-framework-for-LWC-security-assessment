...
definition simon_z3 :: int where
  "simon_z3 = 0b11110000101100111001010001001000000111101001100011010111011011"


definition simon_F_function :: "32 word ⇒ 32 word" where
  "simon_F_function x = xor (and (word_rotl 1 x) (word_rotl 8 x)) (word_rotl 2 x)"


definition simon_encrypt_round :: "32 word ⇒ 32 word × 32 word ⇒ 32 word × 32 word" where
  "simon_encrypt_round k xy = (let (x, y) = xy in (xor (xor k (simon_F_function x)) y, x))"


definition simon_decrypt_round_inverse :: "32 word ⇒ 32 word × 32 word ⇒ 32 word × 32 word" where
  "simon_decrypt_round_inverse k xy_new = (let (x_new, y_new) = xy_new in (y_new, xor (xor x_new k) (simon_F_function y_new)))"


definition simon_rho_const :: "32 word" where
  "simon_rho_const = 0xFFFFFFFC"


function simon_gen_key_schedule_rec :: "32 word list ⇒ nat ⇒ 32 word list" where
  "simon_gen_key_schedule_rec current_keys i = (
     if i ≥ 44 then current_keys
     else
       let z_bit = bit 0b11110000101100111001010001001000000111101001100011010111011011 (i - 4);
...
definition simon_generate_key_schedule :: "32 word list ⇒ 32 word list" where
  "simon_generate_key_schedule initial_keys = simon_gen_key_schedule_rec initial_keys (length initial_keys)"


fun simon_encrypt_iterate :: "32 word × 32 word ⇒ 32 word list ⇒ 32 word × 32 word" where
  "simon_encrypt_iterate st [] = st"
| "simon_encrypt_iterate st (k#ks) = simon_encrypt_iterate (simon_encrypt_round k st) ks"


fun simon_decrypt_iterate :: "32 word × 32 word ⇒ 32 word list ⇒ 32 word × 32 word" where
