...
definition skinny_rounds :: nat where "skinny_rounds = 32"


definition skinny_get_cell :: "nat ⇒ 64 word ⇒ 4 word" where
  "skinny_get_cell i s = ucast (drop_bit (4 * i) s)"
...
definition skinny_sub_cells :: "64 word ⇒ 64 word" where
  "skinny_sub_cells s = foldr (λi acc. 
    skinny_set_cell i (of_nat (skinny_sbox_table ! unat (skinny_get_cell i acc))) acc) [0..<16] s"

definition skinny_sub_cells_inverse :: "64 word ⇒ 64 word" where
  "skinny_sub_cells_inverse s = foldr (λi acc. 
    skinny_set_cell i (of_nat (skinny_sbox_inv_table ! unat (skinny_get_cell i acc))) acc) [0..<16] s"


definition skinny_rc_table :: "6 word list" where
  "skinny_rc_table = map of_nat [0x01, 0x03, 0x07, 0x0F, 0x1F, 0x3E, 0x3D, 0x3B, 0x37, 0x2F, 0x1E, 0x3C, 0x39, 0x33, 0x27, 0x0E, 0x1D, 0x3A, 0x35, 0x2B, 0x16, 0x2C, 0x18, 0x30, 0x21, 0x02, 0x05, 0x0B, 0x17, 0x2E, 0x1C, 0x38]"
...
definition skinny_sr_map :: "nat list" where "skinny_sr_map = [0, 1, 2, 3, 5, 6, 7, 4, 10, 11, 8, 9, 15, 12, 13, 14]"

definition skinny_sr_inv_map :: "nat list" where "skinny_sr_inv_map = [0, 1, 2, 3, 7, 4, 5, 6, 10, 11, 8, 9, 13, 14, 15, 12]"

definition skinny_shift_rows :: "64 word ⇒ 64 word" where
  "skinny_shift_rows s = foldr (λi acc. skinny_set_cell i (skinny_get_cell (skinny_sr_map ! i) s) acc) [0..<16] 0"


definition skinny_shift_rows_inverse :: "64 word ⇒ 64 word" where
  "skinny_shift_rows_inverse s = foldr (λi acc. skinny_set_cell i (skinny_get_cell (skinny_sr_inv_map ! i) s) acc) [0..<16] 0"
...
"skinny_mix_columns s = (
  ...
  in foldr (λ(i,c) acc. skinny_set_cell i c acc) (zip [0..<16] res) 0)"


definition skinny_mix_columns_inverse :: "64 word ⇒ 64 word" where
"skinny_mix_columns_inverse s = (
  let col = λj. [skinny_get_cell j s, skinny_get_cell (j+4) s, skinny_get_cell (j+8) s, skinny_get_cell (j+12) s];
...
definition skinny_tk_permute :: "64 word ⇒ 64 word" where
"skinny_tk_permute tk = (
  let c = λj. skinny_get_cell j tk;
      p = [c 9, c 15, c 8, c 13, c 10, c 14, c 12, c 11, c 0, c 1, c 2, c 3, c 4, c 5, c 6, c 7]
  in foldr (λ(j,v) acc. skinny_set_cell j v acc) (zip [0..<16] p) 0)"

fun skinny_gen_key_schedule :: "64 word ⇒ nat ⇒ 64 word list" where
  "skinny_gen_key_schedule tk 0 = []"
| "skinny_gen_key_schedule tk (Suc n) = tk # skinny_gen_key_schedule (skinny_tk_permute tk) n"
...
definition skinny_decrypt_round_inverse :: "64 word ⇒ nat ⇒ 64 word ⇒ 64 word" where
"skinny_decrypt_round_inverse k r s = 
  skinny_sub_cells_inverse (skinny_add_round_constant r (xor (skinny_shift_rows_inverse (skinny_mix_columns_inverse s)) k))"
...
fun skinny_decrypt_iterate :: "64 word ⇒ 64 word list ⇒ nat ⇒ 64 word" where
  "skinny_decrypt_iterate s ks r = foldl (λacc (k, round). skinny_decrypt_round_inverse k round acc) s (rev (zip ks [0..<32]))"
