...
definition sparx_block_size :: nat where "sparx_block_size = 128"


definition sparx_key_size :: nat where "sparx_key_size = 256"


definition sparx_n_steps :: nat where "sparx_n_steps = 10"


definition sparx_rounds_per_step :: nat where "sparx_rounds_per_step = 4"


definition sparx_word_size :: nat where "sparx_word_size = 16"


definition sparx_n_branches :: nat where "sparx_n_branches = 4"


definition sparx_n_words :: nat where "sparx_n_words = 8"


definition sparx_total_rounds :: nat where 
  "sparx_total_rounds = 10 * 4"


definition sparx_round_key_words :: nat where
  "sparx_round_key_words = 4"
...
"sparx_linear_layer s = (
  if length s = 8 then
    let
      t = sparx_L_w (xor (xor (s ! 0) (s ! 1)) (xor (s ! 2) (s ! 3)))
    in
      [ xor (s ! 4) t, xor (s ! 5) t,
        xor (s ! 6) t, xor (s ! 7) t,
        s ! 0, s ! 1, s ! 2, s ! 3 ]
...
"sparx_linear_layer_inv s = (
  if length s = 8 then
    let
      t = sparx_L_w (xor (xor (s ! 4) (s ! 5)) (xor (s ! 6) (s ! 7)))
    in
      [ s ! 4, s ! 5, s ! 6, s ! 7,
        xor (s ! 0) t, xor (s ! 1) t, 
        xor (s ! 2) t, xor (s ! 3) t ]
...
definition sparx_extract_key_words :: "256 word \<Rightarrow> 16 word list" where
    ...
    let
      ...
      k_next =
        [k0', k1', k2', k3',
         k!4, k!5, k!6, k!7]
    in
      sparx_gen_key_schedule_iterate
...
definition sparx_generate_key_schedule :: "256 word \<Rightarrow> 16 word list" where
  "sparx_generate_key_schedule master_key = (
    let key_words = 256 div 16;
        total_needed = (sparx_total_rounds + 1) * 4;
...
"sparx_apply_encrypt_round state round_keys = (
  if length state = 8 \<and> length round_keys = 4 then
    let
      (a0,a1) = sparx_A_perm_16 (xor (state!0) (round_keys!0))(state!1);
      (a2,a3) = sparx_A_perm_16 (xor (state!2) (round_keys!1))(state!3);
      (a4,a5) = sparx_A_perm_16 (xor (state!4) (round_keys!2))(state!5);
      (a6,a7) = sparx_A_perm_16  (xor (state!6) (round_keys!3))(state!7)
    in
...
definition sparx_apply_decrypt_round ::
  "16 word list \<Rightarrow> 16 word list \<Rightarrow> 16 word list" where
"sparx_apply_decrypt_round state round_keys = (
  if length state = 8 \<and> length round_keys = 4 then
    let
      (s0', s1') = sparx_A_perm_16_inv (state ! 0) (state ! 1);
      s0 = xor s0' (round_keys ! 0);
      (s2', s3') = sparx_A_perm_16_inv (state ! 2) (state ! 3);
      s2 = xor s2' (round_keys ! 1);
      (s4', s5') = sparx_A_perm_16_inv (state ! 4) (state ! 5);
      s4 = xor s4' (round_keys ! 2);
      (s6', s7') = sparx_A_perm_16_inv (state ! 6) (state ! 7);
...
"sparx_encrypt_step_iterate state all_keys round step = (
  if round \<ge> 4 then state
    ...
    in
      sparx_encrypt_step_iterate new_state all_keys (round + 1) step
)"
by pat_completeness auto
termination
  apply (relation "measure (\<lambda>(state, all_keys, round, step). 4 - round)")
  apply auto
  done
...
"sparx_decrypt_step_iterate state all_keys round step = (
  if round \<ge> 4 then state
    ...
    in
      sparx_decrypt_step_iterate new_state all_keys (round + 1) step
)"
by pat_completeness auto
termination
  apply (relation "measure (\<lambda>(state, all_keys, round, step). 4 - round)")
...
definition sparx_block_to_words :: "128 word \<Rightarrow> 16 word list" where
...
definition sparx_words_to_block :: "16 word list \<Rightarrow> 128 word" where
  "sparx_words_to_block words = (
    if length words = 8 then
      foldl (\<lambda>acc i. 
        ...
        in or (push_bit shift word_val) acc) 0 [0..<8]
    else 0)"
...
"sparx_decrypt_steps_iterate state all_keys step = (
  if step \<ge> 10 then state
  else
    let rev_step = 10 - step - 1;
        state_after_rounds =
...
by pat_completeness auto
termination
  apply (relation "measure (\<lambda>(s, k, i). 10 - i)")
  apply auto
  done
...
function sparx_encrypt_steps_iterate :: 
  ...
  by pat_completeness auto
termination
  apply (relation "measure (\<lambda>(state, all_keys, step). 10 - step)")
  apply auto
  done
...
"sparx_encrypt_block plaintext rk_words = (
  let
    ...
    state_after_steps =
      sparx_encrypt_steps_iterate state all_keys 0;
    state_final =
  ...
  in
    sparx_words_to_block state_final
)"
...
"sparx_decrypt_block ciphertext rk_words = (
  ...
  in
    sparx_words_to_block state_after_steps
)"


definition sparx_encrypt :: "128 word \<Rightarrow> 256 word \<Rightarrow> 128 word" where
...
definition sparx_decrypt :: "128 word \<Rightarrow> 256 word \<Rightarrow> 128 word" where
