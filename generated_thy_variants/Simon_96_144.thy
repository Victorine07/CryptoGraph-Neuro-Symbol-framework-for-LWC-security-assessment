...
definition simon_F_function :: "48 word ⇒ 48 word" where
  "simon_F_function x = xor (and (word_rotl 1 x) (word_rotl 8 x)) (word_rotl 2 x)"


definition simon_encrypt_round :: "48 word ⇒ 48 word × 48 word ⇒ 48 word × 48 word" where
  "simon_encrypt_round k xy = (let (x, y) = xy in (xor (xor k (simon_F_function x)) y, x))"


definition simon_decrypt_round_inverse :: "48 word ⇒ 48 word × 48 word ⇒ 48 word × 48 word" where
  "simon_decrypt_round_inverse k xy_new = (let (x_new, y_new) = xy_new in (y_new, xor (xor x_new k) (simon_F_function y_new)))"


definition simon_rho_const :: "48 word" where
  "simon_rho_const = 0xFFFFFFFFFFFC"


function simon_gen_key_schedule_rec :: "48 word list ⇒ nat ⇒ 48 word list" where
  "simon_gen_key_schedule_rec ks i = (if i ≥ 54 then ks else
    let z_bit = bit 0b11110000101100111001010001001000000111101001100011010111011011 (i - 3);
        rs_3 = word_rotr 3 (ks ! (i-1)); rs_1 = word_rotr 1 rs_3;
        new_k = xor (xor (xor (ks ! (i-3)) rs_3) rs_1) (xor (if z_bit then 1 else 0) simon_rho_const)
    in simon_gen_key_schedule_rec (ks @ [new_k]) (i+1))"
  by pat_completeness auto
termination by (relation "measure (λ(ks, i). 54 - i)") auto


definition simon_generate_key_schedule :: "48 word list ⇒ 48 word list" where
  "simon_generate_key_schedule init = simon_gen_key_schedule_rec init (length init)"


fun simon_encrypt_iterate :: "48 word × 48 word ⇒ 48 word list ⇒ 48 word × 48 word" where
  "simon_encrypt_iterate st [] = st" 
| "simon_encrypt_iterate st (k#ks) = simon_encrypt_iterate (simon_encrypt_round k st) ks"


fun simon_decrypt_iterate :: "48 word × 48 word ⇒ 48 word list ⇒ 48 word × 48 word" where
